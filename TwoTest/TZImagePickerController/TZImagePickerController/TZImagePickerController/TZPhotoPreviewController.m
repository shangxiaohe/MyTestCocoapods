...
@interface TZPhotoPreviewController ()<LFPhotoEditingControllerDelegate,UICollectionViewDataSource,UICollectionViewDelegate,UIScrollViewDelegate> {
...
- (void)configBottomToolBar {
    ...
    _editButton.frame = CGRectMake(0, 0, 44, 44);
    _editButton.center = CGPointMake(self.view.tz_width/2, 22);
...
{
    TZAssetModel *model = _models[_currentIndex];
    PHAsset *assetTemp = (PHAsset*)model.asset;
    NSString *filename = [assetTemp valueForKey:@"filename"];
    UIImage* photo;
    NSMutableDictionary *dic = [NSMutableDictionary dictionaryWithDictionary:[[NSUserDefaults standardUserDefaults] objectForKey:@"photoInfo"]];
    if([[dic allKeys] containsObject:filename])
        photo = [UIImage imageWithData:[dic objectForKey:filename]];
...
{
    [self.navigationController popViewControllerAnimated:NO];
    PHAsset *assetTemp = (PHAsset*)[_models[_currentIndex] asset];
    NSString *filename = [assetTemp valueForKey:@"filename"];
    NSData *data = [NSData data];
    /* 判断图片是不是png格式的文件 */
    if (UIImagePNGRepresentation(photoEdit.editPreviewImage))
    {
        /* 返回为png图像 */
        data = UIImagePNGRepresentation(photoEdit.editPreviewImage);
    }
    else
    {
        /* 返回为JPEG图像 */
        data = UIImageJPEGRepresentation(photoEdit.editPreviewImage, 1.0);
    }
    NSMutableDictionary *dic = [NSMutableDictionary dictionaryWithDictionary:[[NSUserDefaults standardUserDefaults] objectForKey:@"photoInfo"]];
    [dic setValue:data forKey:filename];
    [[NSUserDefaults standardUserDefaults] setObject:dic forKey:@"photoInfo"];
